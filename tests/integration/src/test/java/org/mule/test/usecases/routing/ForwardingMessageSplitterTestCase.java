...
public class ForwardingMessageSplitterTestCase extends AbstractIntegrationTestCase {
  ...
  public void testSyncResponse() throws Exception {
    ...
    MuleMessage m = client.request("test://component.1", RECEIVE_TIMEOUT).getRight();
    assertNotNull(m);
    assertThat(m.getPayload(), instanceOf(String.class));
    m = client.request("test://component.2", RECEIVE_TIMEOUT).getRight();
    assertNotNull(m);
    assertThat(m.getPayload(), instanceOf(Integer.class));

    m = client.request("test://error.queue", RECEIVE_TIMEOUT).getRight();
    assertNotNull(m);
